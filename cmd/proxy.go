...
func runProxy(cmd *cobra.Command, args []string) {
	...
	// Initialize the plugin with the subject
	pipeConfig := map[string]string{
		"nats_url":       natsURL,
		"nats_subject":   natsSubject,
		"max_body_bytes": strconv.FormatInt(viper.GetInt64("max-body-bytes"), 10),
	}
...
func setupProxyRoutes(r *gin.Engine, httpClient *http.Client) {
	...
	proxyHandler := func(c *gin.Context) {
		...
		natsSubject := viper.GetString("nats-subject")
		targetURL := fmt.Sprintf("nats://%s%s", natsSubject, c.Request.URL.Path)
		...
		if c.Request.Body != nil {
			...
			if maxBodyBytes > 0 {
				reader = io.LimitReader(c.Request.Body, maxBodyBytes+1)
			}
